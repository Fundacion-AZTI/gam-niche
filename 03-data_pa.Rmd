...
FAO<- st_read(here::here("data", "spatial", "FAO_AREAS.shp"))
...
save(FAO_Atl_no_black_sea, file = here::here("data/spatial/total_area.RData"))
...
# load data-files
#occurrence data
load("C:/use/GitHub/SDM/data/occurrences/occ.RData")
#map Atlantic without black sea
load("C:/use/GitHub/SDM/data/spatial/total_area.RData")

#convert to spatial point data frame
df0 <- data_fus_Atl
names(df0)[3:6]<-c("LON","LAT","YEAR","MONTH") ; df<-df0
coordinates(df)<- ~LON+LAT
crs(df)<-crs(FAO_Atl_no_black_sea)

#convert to sf
Atl.sf<-st_union(st_as_sf(FAO_Atl_no_black_sea))
df.sf<-st_as_sf(df)

ggplot(Atl.sf) + geom_sf() + geom_sf(data=st_union(df.sf))
...
aux <- st_transform(Atl.sf, EPSG_2_UTM)
...
for (yy in sort(unique(df$YEAR))){
  ...
  if (nrow(df.sub)>0){
    buffer.sub <- st_buffer(df.sub, dist=100000)
    buffer.sub <- st_union(buffer.sub)
    aux0.sub <- st_difference(aux, buffer.sub)
    p <- p0 +
      geom_sf(data=buffer.sub, fill="blue")
    ggsave(file.path("plots","pseudo",paste0("area_buffer_",yy,".png")), p, device="png")
  }
}
...
for (yy in sort(unique(df$YEAR))){
  for (mm in sort(unique(df$MONTH))){
    print(paste(yy,mm))
    ...
    if (nrow(df.sub)>0){
      ...
      aux0.sub <- st_difference(aux, buffer.sub)
      p <- p0 +
        geom_sf(data=aux0.sub, fill="red")
...
#fill years and months and in the presence data frame
pseudo$YEAR<-df0$YEAR
pseudo$MONTH<-df0$MONTH
df.sf.utm$MONTH[is.na(df.sf.utm$MONTH)]<-"missingM"
pseudo$MONTH[is.na(pseudo$MONTH)]<-"missingM"

for (yy in sort(unique(df$YEAR))[-c(1:21)]){
  for (mm in c(sort(unique(df$MONTH)),"missingM")){
    print(paste(yy,mm))
    ...
    if (length(idx)>0){
      ...
      p <- p0 +
        ...
        ggtitle(paste(unique(df$scientificName),yy,mm))
...
p <- p0 +
  ...
  geom_sf(data=df.sf, col=4, alpha=0.3)+
  geom_sf(data=pseudo.sf, col=1, shape=4, size=0.5)
print(p)
ggsave(file.path("plots","pseudo","pseudo_all2.png"), p, device="png")
