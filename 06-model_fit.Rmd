...
#use only 50% of data (to get results faster)
data <- dat.all[sample(1:dim(dat.all)[1],floor(dim(dat.all)[1]*0.5)),] 

raw_model_damean <- scam (occurrenceStatus ~  s(BO_sstmean, k=8,bs="cv", m=2)+ s(BO_salinity, k=8,bs="cv", m=2)+ s(BO_damean, k=8,bs="cv", m=2)+ s(BO_sstrange, k=8,bs="cv", m=2) + s(BO_bathymean, k=5, bs="cv",m=2), family=binomial(link="logit"), data=data,sp=rep(0.00001,5))

raw_model_chlomean <- scam (occurrenceStatus ~  s(BO_sstmean, k=8,bs="cv", m=2)+ s(BO_salinity, k=8,bs="cv", m=2)+ s(BO_chlomean, k=8,bs="cv", m=2)+ s(BO_sstrange, k=8,bs="cv", m=2) + s(BO_bathymean, k=5, bs="cv",m=2), family=binomial(link="logit"), data=data,sp=rep(0.00001,5))

AIC(raw_model_chlomean);AIC(raw_model_damean)
summary(raw_model_chlomean)$dev.expl;summary(raw_model_damean)$dev.expl

raw_model<-raw_model_chlomean
